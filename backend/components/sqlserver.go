...
func sqlServerWriterMain(currentNode Node, inputData RequestData) (map[string]interface{}, error) {
	args := config.GetArgs()
	...
	csvToSqlErr := ReadCsvSaveToSQLServer(csvFile, currentNode)
	if csvToSqlErr != nil {
		log.Error("未能正常写入数据库")
		return map[string]interface{}{}, nil
	}
...
func ReadCsvSaveToSQLServer(r io.Reader, currentNode Node) error {
